...
export default {
  ...
  methods: {
    setParallax () {
      if (this.isParallax) {
        this.setProps('background-attachment', 'fixed')
      } else {
        this.setProps('background-attachment', 'scroll')
      }
    },
    changeSize () {
      if (this.backgroundFill === 'normal') {
        this.setProps('background-size', 'auto auto')
      }
      if (this.backgroundFill === 'vertically') {
        this.setProps('background-size', 'auto 100%')
      }
      if (this.backgroundFill === 'horizontally') {
        this.setProps('background-size', '100% auto')
      }
      if (this.backgroundFill === 'both') {
        this.setProps('background-size', 'cover')
      }
      if (this.backgroundFill === 'stretch') {
        this.setProps('background-size', 'contain')
      }
    },
    setPosition (x, y) {
      this.bgPosition = `${x} ${y}`
      this.setProps('background-position', this.bgPosition)
    },
    setProps (prop, value) {
      let s = {}
